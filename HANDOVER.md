...
| `Innovate` | ⭐ Pinned "Featured Work" scatter on ≥1024px. **<1024px (client request 2026-08-17, `MobileInnovate`):** pinned list — heading (no button) first, then the four stat cards slide in alternately from the left / right as you scroll (layer pans up to follow), and the "Become a Client" button arrives last. The old "20+ Global Awards & Features" tile is now **"10+ Websites Launched"** (`LaunchedCard`, rocket icon) on all viewports — alternatives offered: "9 Industries Served", "24h Response Time", "100% Hand-Coded". Timings in `MP_START` / `MP_STEP` / `MP_DUR`. |

...
| `HeroShowcase` | ⭐ Pinned scroll-morph "Latest Projects" — **matches Figma exactly** on ≥640px (4 cards: BVC Logistics, Trippy Tour, IVVYLISTIC, Amorada with `lp-screen-*.jpg`); each card links to its case-study route. Do NOT swap the card artwork — client insisted the Figma design stays. **Phones (<640px) use a different choreography (client request 2026-08-17, `MobileDeck` in the same file):** no "Book a call with us" CTA in the copy (the floating pill covers it); the four cards sit under the paragraph as a pile with BVC on top; scrolling un-stacks them one at a time (Trippy → Ivylistic → Amorada) into a vertical list, captions fading in as each is revealed, while the whole layer pans up so the newest card is on screen. The remaining pile travels as one unit each step (the leader slides to its slot, the cards behind ride underneath) so nothing peeks out under the card above. Timing windows in `DECK_STEPS`, pile look in `PILE_DY`/`PILE_DSCALE`/`PILE_ROT`. |
...
| `ShowreelVideo` | **HIDDEN on all viewports (client request 2026-08-17)** — unmounted from `page.tsx`, component file kept. Was: 16:9 video slot placeholder — drop `public/showreel.mp4` and flip `HAS_VIDEO` in the component, then re-import it in `page.tsx`. |
| `Innovate` | ⭐ Pinned "Featured Work" scatter on ≥1024px. **<1024px (client request 2026-08-17, `MobileInnovate`):** pinned list — heading (no button) first, then the four stat cards slide in alternately from the left / right as you scroll (layer pans up to follow), and the "Become a Client" button arrives last. The old "20+ Global Awards & Features" tile is now **"50+ Websites Launched"** (`LaunchedCard`, rocket icon, client figure) and "5K+ Global Project" is now **"₹50Cr+ Revenue Generated"** ("E-commerce and B2B brands we've built for have generated…", `TrustedCard`, client figure) on all viewports. Timings in `MP_START` / `MP_STEP` / `MP_DUR`. |
| `OurServices` | ⭐ Pinned scrollytelling on ≥640px: zooms in on the first point, reveals 7 points one-by-one along a growing horizontal navy line, zooms out after the last. **Phones (<640px, client pick 2026-08-17, `MobileTimeline`):** vertical timeline — the navy line runs down the left edge and grows on scroll, each service lights up at its node (title + description slide in from the right, newest node glows), and the layer pans to keep the active item ~36% down the screen (`TL_ANCHOR`). Item offsets are measured from the DOM (re-measured after webfonts), so descriptions can be any length. Static list only for reduced motion. |

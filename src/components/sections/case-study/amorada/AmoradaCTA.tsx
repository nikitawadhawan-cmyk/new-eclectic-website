/**
 * AmoradaCTA — Figma node 216:4845 ("Ready For Similar Results?" CTA)
 *
 * Full-width rounded band (47px radius, 1501×825 in Figma): a 5×3 photo-grid
 * collage of amorada home-linen product shots (beds, table settings, folded
 * linens, cushions, resin trays) under a navy tint (rgba(42,49,95,0.84)),
 * with centred white content:
 *
 ...
 *
 * ASSET APPROACH (matches sister CaseCTA.tsx): the collage is exported as
 * ONE raster — public/figma/am-cta-collage.jpg (1600px, jpeg q72, verified) —
 * laid as an absolutely-positioned <img> with the navy tint + live text over
 * it. Uses assetPath() for the raw <img> (GitHub Pages basePath).
 *
 * DEVIATIONS (flagged in report):
 *  - Heading 128px desktop scaled down mobile-first (no mobile frame in Figma).
 *  - Subline's Figma line box is 140.8px (container artifact); rendered at a
 *    sane ~1.4 leading instead.
 *  - "GET IN TOUCH" button target is a placeholder "#contact" anchor.
 *  - The button's Figma arrow icon sits outside the pill's clip (hover-state
 *    leftover) and is invisible in the rendered design — omitted here to
...
export default function AmoradaCTA() {
  return (
    <section className="w-full py-8 lg:py-12">
      <div className="mx-auto w-full max-w-[1500px] px-4 sm:px-6 lg:px-10">
        <div className="relative overflow-hidden rounded-[28px] lg:rounded-[47px]">
          {/* Background collage (single raster of the 5×3 linen photo grid) */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={assetPath("/figma/am-cta-collage.jpg")}
            alt=""
            aria-hidden="true"
